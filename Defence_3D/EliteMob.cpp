...
AEliteMob::AEliteMob() {
	...
	CollisionComp->OnComponentHit.AddDynamic(this, &AEliteMob::OnHit);
	CollisionComp->OnComponentBeginOverlap.AddDynamic(this, &AEliteMob::OnBeginOverlap);

	RootComponent = CollisionComp;

	MeshComponent = CreateDefaultSubobject<UStaticMeshComponent>(TEXT("Mesh"));
	MeshComponent->AttachToComponent(RootComponent, FAttachmentTransformRules::KeepWorldTransform);
	MeshComponent->SetRelativeLocation(FVector(1.0f, -16.0f, -93.0f));
	MeshComponent->SetRelativeScale3D(FVector(7.0f, 7.0f, 7.0f));
	MeshComponent->SetCollisionProfileName(FName("Enemy"));

	HealthComponent = CreateDefaultSubobject<UHealthComponent>(TEXT("health"));
	HealthComponent->SetMaxHealth(400);
	...
	HPBarWidget->SetRelativeLocation(FVector(0.0f, 0.0f, 160.0f));
...
// Called every frame
void AEliteMob::Tick(float DeltaTime)
{
...
void AEliteMob::BeginPlay()
{
	Super::BeginPlay();

}

void AEliteMob::OnHit(UPrimitiveComponent* HitComp, AActor* OtherActor, UPrimitiveComponent* OtherComp, FVector NormalImpulse, const FHitResult& Hit)
{
...
void AEliteMob::OnBeginOverlap(UPrimitiveComponent* OverlappedComp,
	AActor* OtherActor,
